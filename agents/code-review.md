...
**Start:** `--agent-name code-review`, `--feature-id {NNN}`, `--input-mode structured_feature`, `--input-summary "Code review for {NNN} feature-name"`.
...
**Always log, before closing — not conditional on anything going wrong:** an `input_quality` reflection rating the engineer report. Rate 1-10 and name what made it easy or hard to review from — did "For the Review Agents" actually name the areas worth scrutiny, or did you have to find them yourself; see the `agent-log` skill for the exact format.

**Log events for:** test run (`bash`), report written (`file_write`).
