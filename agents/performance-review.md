...
- An assumption traces to a hole in the spec or engineer report — it didn't say enough to judge the performance impact cleanly — log as `decision --type gap`, naming which artifact fell short; this feeds `rails-log-analyst`'s Pattern Type 3
...
**Always log, before closing — not conditional on anything going wrong:** an `input_quality` reflection rating the engineer report. Rate 1-10 and name what made it easy or hard to review from — did it name the performance-sensitive areas worth scrutiny, or did you have to find them yourself; see the `agent-log` skill for the exact format.
