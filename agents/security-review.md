...
**Before closing the run**, log a `reflection --type struggle` for each entry in the report's "Where I struggled" section — anything where the authorization model was ambiguous, where evaluating a finding required judgment beyond clear guidelines, or where the review was harder than expected:
