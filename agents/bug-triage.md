...
Read `team.yml` at the project root per the `github-cli` skill. If it's missing, stop and tell the user to run `/bug` or `/request` once first — that's what creates it.
