...
3. **Pre-migration check** — Before writing any migration, verify every new `belongs_to`/`references` column has a corresponding `foreign_key: true` (or explicit `add_foreign_key`) in the same migration, and that the owning model gets its reciprocal `has_many`/`has_one` if the association is meant to be traversed both ways. A downstream project found missing FK constraints and missing reciprocal associations to be the single highest-severity-rate finding category in its code-review history — recurring across five distinct features, with the highest NEEDS WORK rate of any category examined. Catching it before the migration runs is free; catching it in review costs a round.
4. **Red** — Write the failing test first. Run it. Confirm it fails for the right reason.
5. **Green** — Write the minimum implementation to make the test pass. Run it. Confirm it passes.
6. **Refactor** — Once green: check for naming improvements, concern extraction opportunities, N+1 queries. Run tests again after refactoring.
7. **Validate** — Run `bin/ci` if the project has one (`test -f bin/ci`; Rails 8+ apps ship it — check `config/ci.rb` for the exact steps it runs, typically `bin/rubocop`, `bin/bundler-audit`, `bin/importmap audit`, `brakeman`, and `bin/rails test` together). If there's no `bin/ci`, run the equivalent individually: `bin/rubocop`, `bin/rails test`, `brakeman`. Walk branches: every conditional has a test.
...
- [idea] [needs-discovery | tech-debt | bug] — [what surfaced it, one sentence]
