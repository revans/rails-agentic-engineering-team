...
- **`status: "ok"`** — present each non-empty category, in this order:
  ...
  3. **`conflicts`** — both the local file and upstream changed since the last sync, or (`base_sha: null`) there's no sync history *from this team* for a file that already exists locally with different content — either because something was placed at that path outside this mechanism, or, for a genuinely shared file (`bin/agent-log`, `bin/team-setup-git`/`-gh`/`-sqlite`), because a *different* team already vendored its own copy there (see `docs/updates.md`, "Keeping Shared Tools in Sync" — `team.lock.yml` is namespaced per team specifically so this shows up as a conflict instead of a silent overwrite). For each: `diff -u "$TARGET_DIR/{path}" "{snapshot_dir}/{path}"`, show it, then ask: take upstream, keep local, or skip for now. If the file carries a `SHARED_TOOL_VERSION` constant (`bin/agent-log` does), compare that first — taking whichever side has the higher version is the right default unless the diff itself looks wrong.
