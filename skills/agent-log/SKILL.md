...
**Never invoke `git stash` in any form — bare, scoped, or path-limited — in a shared worktree while `bin/agent-log` writes might be pending, or for mutation-testing isolation against a fix that may already be committed.** `db/agent_log.sqlite3` is a git-tracked file that `bin/agent-log` writes to directly, outside git's own awareness — a bare `git stash` stashes it along with everything else, and a later `git stash pop`/`git checkout --ours` only restores it to *some* valid git state, not necessarily the one with your own most recent row-level writes intact. Scoping the push to specific paths (`git stash -- path/to/file.rb`) does not fix this: in bug-fix-mode review, the change under test is normally already committed, so the target path has no working-tree diff — `git stash push` then silently no-ops ("No local changes to save," not an error) instead of creating an entry. A `git stash pop` run immediately after, on the assumption "restore what I just pushed," has no way to detect that nothing was pushed — it pops whatever is on top of the stash stack, which is **shared repo-wide across every concurrent worktree on this shared `.git`**, not scoped to your worktree or session. Confirmed root cause of repeated merge conflicts in `db/agent_log.sqlite3`, at least one lost completed review report, and at least one lost agent-log run, across six distinct incidents in a single day on a downstream project. **For mutation testing or reverting a single file, never reach for `git stash` at all.** Use one of: (a) the Edit tool directly, applying and reversing the change by hand; (b) `git diff > /tmp/scratch.patch` on the target path, `git checkout -- <path>` to revert, then `git apply /tmp/scratch.patch` to restore — this never touches the shared stash stack; (c) `git show <parent-commit>:<path>` to read the pre-change version without touching the working tree at all, when you only need to compare, not actually revert.
