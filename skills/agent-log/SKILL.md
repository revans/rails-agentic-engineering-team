...
**Never run a bare `git stash` (or `git checkout .` / `git reset --hard` / any other broad, unscoped git operation) in a worktree while `bin/agent-log` writes might be pending.** `db/agent_log.sqlite3` is a git-tracked file that `bin/agent-log` writes to directly, outside git's own awareness — a bare `git stash` stashes it along with everything else, and `git stash pop`/`git checkout --ours` afterward only restores the file to *some* valid git state, not necessarily the one with your own most recent row-level writes still in it. `git stash list` and "no conflict markers" both look clean even when this has happened — the loss is silent and doesn't show up until a later `bin/agent-log query` comes back missing rows a report already cites by ID. If you need to stash or reset for an unrelated reason mid-review, scope the operation to specific paths (`git stash -- path/to/file.rb`, never a bare `git stash`) and leave `db/agent_log.sqlite3` out of it entirely — its own writes are already durable via SQLite's own commit, not something git needs to manage for you.
