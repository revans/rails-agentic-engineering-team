...
**Even a single, correctly-formatted `Closes #N` in the PR body has been observed to silently not fire on merge** — the exact mechanism isn't fully pinned down (it isn't simply "squash merges use the commit list instead of the PR body," since plenty of squash-merged PRs closed correctly off that same pattern), so don't assume a specific cause and don't assume it's fixed by rewording. Treat `gh issue view {N} --json state` after *every* merge — not just multi-issue ones — as a required step, not a spot-check: if it's still open, close it by hand with `gh issue close {N}` rather than re-merging or investigating further in the moment.
