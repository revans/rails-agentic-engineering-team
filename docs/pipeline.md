...
- As the very last step, the orchestrator checks how many completed cycles — feature or bug fix — have piled up since `rails-log-analyst` last ran and mentions it in the final report once that reaches the configured threshold (`cadence.log_analyst_interval` in `team.yml`; 15 by default) — it never runs `rails-log-analyst` for you, only tells you when it's worth doing yourself.
