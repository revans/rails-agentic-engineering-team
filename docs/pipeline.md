...
- Any agent can notice something that should exist but isn't part of the current feature. Rather than build it or let it evaporate, it names the idea in its own report; at pipeline completion the orchestrator sweeps every report and adds new ones to `TODO.md`'s **New Features to Discover** section. See the `scope-capture` skill.
